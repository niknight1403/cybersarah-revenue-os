...
export function AutonomyCycleStatus({ status }: { status: AutonomyCycleStatus }) {
  ...
  const tone = status === "failed" ? "border-rose-200/20 bg-rose-300/10 text-rose-100" : status === "duplicate" ? "border-amber-200/20 bg-amber-300/10 text-amber-100" : "border-cyan-200/20 bg-cyan-300/10 text-cyan-100";
...
export default function AutonomyTasks() {
  ...
  const startCycle = trpc.growth.startAutonomyCycle.useMutation({ onMutate: () => setCycleStatus("running"), onSuccess: result => { setCycleStatus(result.duplicate ? "duplicate" : "started"); void utils.revenue.overview.invalidate(); void utils.growth.status.invalidate(); }, onError: () => setCycleStatus("failed") });
  if (overview.isError || growth.isError) return <RevenueQueryError subject="Die autonomen Tasks" />;
