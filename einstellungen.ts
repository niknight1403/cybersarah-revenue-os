...
// ─── API-KEY-AGENT ROUTEN ────────────────────────────────────────────────────
import {
  holeAlleApiKeyStatus,
  speichereApiKey,
  testeApiKey,
  sucheNeueApis,
  scannApiIntegrationen,
  BEKANNTE_API_SLOTS,
} from "../agents/apiKeyAgent";

// Alle API-Key-Status abrufen
router.get("/einstellungen/api-keys", async (_req, res) => {
  try {
    const scan = await scannApiIntegrationen();
    res.json(scan);
  } catch (err) {
    logger.error({ err }, "API-Key-Scan fehlgeschlagen");
    res.status(500).json({ error: "Scan fehlgeschlagen" });
  }
});

// Einzelnen API-Key speichern + sofort testen
router.post("/einstellungen/api-keys/:schluessel", async (req, res) => {
  const { schluessel } = req.params;
  const { wert } = req.body as { wert: string };

  if (!wert || wert.length < 8) {
    res.status(400).json({ error: "Key zu kurz oder leer" });
    return;
  }

  const slot = BEKANNTE_API_SLOTS.find(s => s.schluessel === schluessel);
  if (!slot) {
    res.status(404).json({ error: "Unbekannter API-Key-Slot" });
    return;
  }

  await speichereApiKey(schluessel, wert);

  // OpenAI Key sofort in Runtime aktivieren
  if (schluessel === "OPENAI_API_KEY") {
    await aktualisiereApiKey(wert);
    await aktualisiereVerfuegbarkeit();
  }

  // Sofort testen
  const testResult = await testeApiKey(slot, wert);

  res.json({ gespeichert: true, getestet: true, ...testResult });
});

// Einzelnen Key testen ohne zu speichern
router.post("/einstellungen/api-keys/:schluessel/testen", async (req, res) => {
  const { schluessel } = req.params;
  const { wert } = req.body as { wert?: string };

  const slot = BEKANNTE_API_SLOTS.find(s => s.schluessel === schluessel);
  if (!slot) {
    res.status(404).json({ error: "Unbekannter Slot" });
    return;
  }

  // Gespeicherten Key nutzen wenn keiner angegeben
  const key = wert ?? process.env[schluessel] ?? null;
  if (!key) {
    res.status(400).json({ gueltig: false, nachricht: "Kein Key vorhanden" });
    return;
  }

  const result = await testeApiKey(slot, key);
  res.json(result);
});

// Neue APIs per KI suchen
router.get("/einstellungen/api-keys/neue-suchen", async (_req, res) => {
  try {
    const empfehlungen = await sucheNeueApis();
    res.json({ empfehlungen });
  } catch (err) {
